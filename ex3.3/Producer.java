...
public class Producer implements Runnable {
    ...
    public Producer(ArrayBlockingQueue<Message> q, int n) {
	queue = q;
	id = n;
    }
	...
	while (running) {
		...
		queue.offer(msg,5,  TimeUnit.SECONDS); // Put the message in the queue
		count++;
	...
	try {
	    queue.offer(msg, 5,  TimeUnit.SECONDS); // Put this final message in the queue
	} catch (InterruptedException e) {
	    e.printStackTrace();
	}
